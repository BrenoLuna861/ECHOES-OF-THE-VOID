...
#define SURVIVAL_TIME 30 // Tempo de sobreviv�ncia em segundos
...
int checkCollisionWithPoints(float newX, float newY) {
    for (int i = 0; i < numPoints; i++) {
        ...
        if (distance < PLAYER_RADIUS + POINT_RADIUS) { // Ajuste de colis�o
            return 1;
        }
    }
    return 0;
...
void updateSurvivalTimer(int value) {
    if (!showMenu && player.alive) {
        survivalTimer += 0.1f; // Incrementa o tempo de sobreviv�ncia
...
void keyboardUp(unsigned char key, int x, int y) {
    // Desativa a dire��o quando a tecla � liberada
    switch (key) {
...
void initMonsters() {
    for (int i = 0; i < currentMonsterCount; i++) {
        ...
        while (!validPosition) {
            ...
            validPosition = 1; // Assume que a posi��o � v�lida

            // Verifica se a posi��o gerada colide com algum ponto (bola verde)
            for (int j = 0; j < numPoints; j++) {
                ...
                if (distance < MONSTER_RADIUS + POINT_RADIUS) {
                    validPosition = 0; // Colidiu, ent�o a posi��o n�o � v�lida
                    break;
                }
            }
        }
